...
def fit_bicubic_to_image(image: np.ndarray, k: int) -> tuple[list[list[np.ndarray]], np.ndarray]:
    ...
    return coeffs, np.asarray(sigmas)


def reconstruct_image_from_coefficients(coefficients: list[list[np.ndarray]]) -> tuple[np.ndarray, dict[tuple[int, int], np.ndarray]]:
